...
export default function CommentsList({filmId, comments}: CommentsListProps){
    ...
    return (
        <div>
            <h2 className="text-3xl mb2">Community Comments</h2>
            <div className="flex flex-col space-y-4 my-3">
                {comments?.map((comment) => (
                    <div className="p-4 rounded border border-slate-400">
                        ...
                        <p className="text-gray-700 text-xl mb-2">{comment.message}</p> 
                    </div>
                ))}
                <div className="p-4 rounded border border-slate-400">
                    <Form method="post">
                        <fieldset disabled={transition.state === 'submitting'}>
                            <div className="inline-block m-y">Name:</div>
                            <input
                                name="name"
                                type="text" 
                            ...
                            {actionData?.errors?.message && (<p className="text-red-500">{actionData?.errors?.message}</p>)}
                            <button 
                                type="submit"
